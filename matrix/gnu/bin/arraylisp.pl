...
Array::APX - Array Programming eeleeeeXtensions
...
of Array::Deep-utils and overloads quite some basic Perl operators in a way
...
archetypal APL solution. The basic idea is this: Create an outer product
...
# Overload everything defined in %special_binary_operates:
...
sub outer
{
    ...
    # the initial stage of a |...|-operator and have to remember the
    ...
    croak 'outer: Strange parameter-types: >>', ref($left),
          '<< and >>', ref($right), '<<';
}
...
sub reduce
{
    ...
    croak 'outer: Strange parameter-types: ', ref($_[0]), ' and ', ref($_[0]);
}
...
shows both usages of rho:
    ...
    print 'Dimension vector of this result: ', $z->rho();
...
This test program yields the following output:
    ...
    Dimension vector of this result: [    3    3 ]
...
+1 and -1 positions respectively:
...
sub slice
{
    ...
    return bless Array::DeepUtils::copy([@$data], [@$control]);
}
...
sub _stringify {
    ...
    while ( @stack ) {
        ...
        if ( $i <= $#$e  ) {
            ...
            $txt .= defined($e->[$i]) ? sprint("%4s", $e->[$i]) : 'undef';
        }
...
Array::APX relies mainly on Array::Deep-utils which, in turn, was developed
for the interpreter of the array programming language Lang5. The source of
Array::Deep-utils is maintained in the source repository of Lang. In addition
...
Bernd Uhlmann E<lt>summand@taxman.deE<gt>

Thomas Katz E<lt>took@cpan.orgE<gt>

=head1 COPYRIGHT

Copyright (C) 2012 by Bernd Uhlmann, Thomas Katz
